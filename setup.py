...
INSTALL_REQUIRES = [
    'matplotlib', 'nipype', 'nibabel', 'pandas', 'pyxnat', 'scipy', 'httplib2'
]
...
def check_dependencies():
    ...
    if missing_deps:
        missing = (", ".join(missing_deps).replace(
            "sklearn", "scikit-learn").replace("skimage", "scikit-image"))
...
if __name__ == "__main__":
    ...
    if not (len(sys.argv) >= 2 and
            ('--help' in sys.argv[1:] or sys.argv[1] in
             ('--help-commands', '--version', 'egg_info', 'clean'))):
        check_dependencies()
